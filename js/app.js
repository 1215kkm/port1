...
class PortfolioRenderer {
    ...
    createItemHTML(item) {
        ...
        if (hasDetailContent) {
            detailButtonHTML = `<button class="btn btn-outline" onclick="window.showPortfolioDetail && window.showPortfolioDetail('${item.id || ''}')">상세설명</button>`;
        } else if (hasThumbnailOnly) {
            detailButtonHTML = `<button class="btn btn-outline" onclick="window.showImageModal && window.showImageModal('${addCacheBuster(thumbnails[0])}', '${(item.title || '').replace(/'/g, "\\'")}')">상세설명</button>`;
        }
    ...
    adjustSingleThumbnails() {
        ...
        items.forEach(item => {
            ...
            const checkOrientation = () => {
                ...
                if (w > h) {
                    item.classList.add('thumb-landscape');
                } else {
                    // portrait or square
                    item.classList.add('thumb-portrait');
                }
            };
        ...
        if (customLinksContainer) {
            if (customLinks.length > 0) {
                customLinksContainer.innerHTML = customLinks.map(link =>
                    `<a href="${link.url}" target="_blank" rel="noopener noreferrer" class="custom-link-item"><svg class="external-link-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"></path><polyline points="15 3 21 3 21 9"></polyline><line x1="10" y1="14" x2="21" y2="3"></line></svg>${link.label}</a>`
                ).join('');
